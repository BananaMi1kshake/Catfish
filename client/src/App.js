      ...
      <div style={{...styles.mainContainer, opacity: isFading ? 0 : 1 }}>
        <div style={{ paddingBottom: '60px' }}>
          {(() => {
            switch (gameState) {
              case 'Intermission':
                return <div style={styles.container}><h1>Round {intermissionData?.round}</h1><div style={{...styles.card, margin: '20px auto'}}><h2 style={{textAlign: 'center'}}>Current Scores</h2>{intermissionData?.players.sort((a,b) => b.score - a.score).map((p) => (<p key={p.id} style={{fontSize: '1.5em', margin: '10px 0', color: styles.colors.text}}>{p.name}: <strong>{p.score}</strong></p>))}</div></div>;
              case 'GameOver':
                return <div style={styles.container}><h1>Game Over!</h1><div style={{...styles.card, margin: '20px auto'}}><h2 style={{textAlign: 'center'}}>Final Results</h2>{finalScores.sort((a,b) => b.score - a.score).map((p, index) => (<p key={p.id} style={{fontSize: '1.5em', margin: '10px 0', color: styles.colors.text}}>{index === 0 ? <FaTrophy style={{color: '#f1c40f', marginRight: '10px'}}/> : ''} {p.name}: <strong>{p.score}</strong></p>))}<button onClick={handlePlayAgain} style={{...styles.button, marginTop: '20px', width: '100%'}}>Play Again</button></div></div>;
              case 'Reveal':
                ...
                if (slide.type === 'voting') {
                  ...
                  return <div style={styles.container}><h1>Vote for the Best Catfish!</h1><p>Each vote a player receives is worth 200 points.</p><div style={{marginTop: '20px', display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: '10px'}}>{slide.players.filter(p => p.id !== socket.id).map(p => (<button key={p.id} onClick={() => handleVote(p.id)} style={{...styles.button, padding: '20px', fontSize: '1.2em'}}>{p.name}</button>))}</div></div>;
                }
                return <div style={styles.container}><h1>The Reveal</h1><div style={{...styles.card, margin: '20px auto', minHeight: '300px', display: 'flex', flexDirection: 'column', justifyContent: 'center'}}>{slide.type === 'intro' && <h2>{slide.creatorName} was trying to catfish... <strong>{slide.targetName}!</strong></h2>}{slide.type === 'profile' && <div><h3>They created this profile:</h3><div style={{display: 'flex', alignItems: 'center'}}><img src={slide.profile?.imageUrl} alt={slide.profile?.fakeName} style={styles.profilePic} /><h4>{slide.profile?.fakeName}</h4></div><p><strong>Bio:</strong> {slide.profile?.bio}</p><p><strong>Likes:</strong> {slide.profile?.likes.join(', ')}</p><p><strong>Dislikes:</strong> {slide.profile?.dislikes.join(', ')}</p></div>}{slide.type === 'sabotageReveal' && <div><h3><FaSkullCrossbones style={{marginRight: '10px'}}/>But wait, there was sabotage!</h3><p><strong>{slide.sabotageInfo?.sabotagerName}</strong> changed the <strong>{slide.sabotageInfo?.field}</strong> from "{slide.sabotageInfo?.oldValue}" to "{slide.sabotageInfo?.newValue}"!</p></div>}{slide.type === 'chatPlayback' && <ChatReveal conversation={slide.conversation} creatorId={slide.creatorId} onComplete={advanceSlide} />}{slide.type === 'decision' && <div><h2>{slide.targetName}'s decision was...</h2><h1 style={{color: slide.decision === 'agree' ? styles.colors.success : styles.colors.danger}}>{(slide.decision || 'no decision').toUpperCase()}!</h1></div>}{slide.type === 'score' && <h2>{slide.creatorName} gets <strong style={{color: styles.colors.accent}}>{slide.score}</strong> points for that!</h2>}</div></div>;
              case 'Decision':
                if (hasDecided) return <div style={styles.container}><h1>Decision Locked In!</h1><p>Waiting for the timer...</p></div>;
                return <div style={styles.container}><h1>The Decision</h1><p>After chatting with "{myCatfishProfile?.fakeName}", do you agree to a date?</p><div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginTop: '30px' }}><button onClick={() => handleDecision('agree')} style={{ ...styles.button, background: styles.colors.success, padding: '20px 40px', fontSize: '1.5em' }}><FaCheckCircle style={{marginRight: '10px'}}/> AGREE</button><button onClick={() => handleDecision('reject')} style={{ ...styles.button, background: styles.colors.danger, padding: '20px 40px', fontSize: '1.5em' }}><FaTimesCircle style={{marginRight: '10px'}}/> REJECT</button></div></div>;
              case 'Chat':
                ...
                return <div style={styles.container}><h1>Mingle & Chat</h1><div style={styles.chatContainer}><div style={styles.chatTabs}><button onClick={() => handleSetActiveChat('catfish')} style={activeChat === 'catfish' ? styles.activeTab : styles.tab}>Chat with Target ({target?.name}) {unreadAsCatfish > 0 && <span style={styles.unreadBadge}>{unreadAsCatfish}</span>}</button><button onClick={() => handleSetActiveChat('target')} style={activeChat === 'target' ? styles.activeTab : styles.tab}>Chat with Catfish ({myCatfishProfile?.fakeName}) {unreadAsTarget > 0 && <span style={styles.unreadBadge}>{unreadAsTarget}</span>}</button></div><div style={styles.chatWindow}><div style={styles.chatHeader}>{activeChat === 'target' && myCatfishProfile?.imageUrl && (<img src={myCatfishProfile.imageUrl} alt={myCatfishProfile.fakeName} style={styles.chatHeaderPic} />)}<span>Talking to: {chatPartnerName}</span></div><div ref={chatWindowRef} style={styles.messagesArea} onScroll={() => { const cw = chatWindowRef.current; if (cw.scrollHeight - cw.scrollTop <= cw.clientHeight + 5) setShowScrollDown(false); }}>{currentChatMessages.map((msg, index) => (<div key={index} style={msg.senderId === socket.id ? styles.myMessage : styles.theirMessage}>{msg.text}</div>))}</div>{showScrollDown && <button onClick={handleScrollDown} style={styles.scrollDownButton}>↓ New Messages</button>}<form onSubmit={handleSendMessage} style={styles.messageForm}><input type="text" value={currentMessage} onChange={(e) => setCurrentMessage(e.target.value)} placeholder="Type a message..." style={styles.messageInput}/><button type="submit" style={styles.sendButton}>Send <FaPaperPlane style={{marginLeft: '8px'}}/></button></form></div></div></div>;
              case 'Sabotage':
                ...
                return <div style={styles.container}><h1><FaSkullCrossbones style={{marginRight: '10px'}}/>Sabotage! ({sabotagesRemaining} left)</h1><p>Click items on other profiles to change them.</p><div style={styles.profilesContainer}>{Object.values(allProfiles).filter(p => p.creatorId !== socket.id).map(profile => (<div key={profile.creatorId} style={styles.card}><div style={{display: 'flex', alignItems: 'center', gap: '10px'}}><img src={profile.imageUrl} alt={profile.fakeName} style={{...styles.profilePic, width: '50px', height: '50px', cursor: 'pointer'}} onClick={() => handleSabotage(profile.creatorId, 'imageUrl', profile.imageUrl)}/><h3 style={styles.sabotageField} onClick={() => handleSabotage(profile.creatorId, 'fakeName', profile.fakeName)}>{profile.fakeName} <FaUserSecret/></h3></div><p style={styles.sabotageField} onClick={() => handleSabotage(profile.creatorId, 'bio', profile.bio)}><strong>Bio:</strong> {profile.bio}</p><p><strong>Likes:</strong></p><ul style={{ paddingLeft: '20px' }}>{profile.likes.map((like, index) => (<li key={index} style={styles.sabotageField} onClick={() => handleSabotage(profile.creatorId, 'likes', like, index)}>{like}</li>))}</ul><p><strong>Dislikes:</strong></p><ul style={{ paddingLeft: '20px' }}>{profile.dislikes.map((dislike, index) => (<li key={index} style={styles.sabotageField} onClick={() => handleSabotage(profile.creatorId, 'dislikes', dislike, index)}>{dislike}</li>))}</ul></div>))}</div></div>;
              case 'ProfileCreation':
                if (isProfileSubmitted) return <div style={styles.container}><h1>Profile Submitted!</h1><p>Waiting for the timer...</p></div>;
                return <div style={styles.container}><h1>Create a Profile for {target?.name}</h1><form onSubmit={handleProfileSubmit} style={styles.form}><div style={styles.imageSearchContainer}><div style={styles.imagePreview}>{imageUrl ? <img src={imageUrl} alt="Selected" style={{width: '100%', height: '100%', objectFit: 'cover'}}/> : 'Select a Pic'}</div><div style={{width: '100%', display: 'flex', gap: '5px'}}><input type="text" placeholder="Search online..." value={imageSearchTerm} onChange={e => setImageSearchTerm(e.target.value)} style={{...styles.input, width: '50%'}} onKeyDown={(e) => { if (e.key === 'Enter') handleImageSearch(e); }}/><button type="button" onClick={handleImageSearch} style={{...styles.button, width: '25%'}}><FaSearch/></button><button type="button" onClick={handleUploadClick} style={{...styles.button, width: '25%', background: '#6c757d'}}><FaUpload/></button></div><input type="file" ref={fileInputRef} style={{ display: 'none' }} onChange={handleFileSelect} accept="image/png, image/jpeg" /></div><input type="text" placeholder="Fake Name" value={fakeName} onChange={e => setFakeName(e.target.value)} required style={styles.input} /><textarea placeholder="Fake Bio..." value={bio} onChange={e => setBio(e.target.value)} style={{...styles.input, height: '80px', resize: 'vertical'}} /><p>3 Likes:</p><input type="text" placeholder="Like #1" value={likes[0]} onChange={e => handleLikeChange(0, e.target.value)} required style={styles.input} /><input type="text" placeholder="Like #2" value={likes[1]} onChange={e => handleLikeChange(1, e.target.value)} required style={styles.input} /><input type="text" placeholder="Like #3" value={likes[2]} onChange={e => handleLikeChange(2, e.target.value)} required style={styles.input} /><p>3 Dislikes:</p><input type="text" placeholder="Dislike #1" value={dislikes[0]} onChange={e => handleDislikeChange(0, e.target.value)} required style={styles.input} /><input type="text" placeholder="Dislike #2" value={dislikes[1]} onChange={e => handleDislikeChange(1, e.target.value)} required style={styles.input} /><input type="text" placeholder="Dislike #3" value={dislikes[2]} onChange={e => handleDislikeChange(2, e.target.value)} required style={styles.input} /><button type="submit" style={{...styles.button, width: '100%', marginTop: '10px'}}>Submit Profile</button></form></div>;
              case 'Assignment':
                return <div style={styles.container}><h1>The Assignment</h1><p style={{ fontSize: '1.2em' }}>Your target is...</p><div style={styles.targetBox}><h2 style={{ margin: 0 }}>{target?.name}</h2></div></div>;
              case 'Lobby':
              default:
                if (!players.some(p => p.id === socket.id)) {
                  return <div style={styles.container}><h1>Join the Catfish Lobby</h1><form onSubmit={(e) => {e.preventDefault(); handleJoinLobby();}} style={styles.form}><input type="text" placeholder="Enter your name" value={name} onChange={(e) => setName(e.target.value)} style={styles.input} /><button type="submit" style={styles.button}>Join Game <FaUserPlus style={{marginLeft: '8px'}}/></button></form></div>;
                }
                return <div style={styles.container}><h1>Welcome to the Lobby, {name}!</h1><h3>Players Waiting ({players.length}):</h3><ul style={{ listStyle: 'none', padding: 0 }}>{players.map((p) => (<li key={p.id} style={styles.playerListItem}>{p.id === socket.id ? `(You) ${p.name}` : p.name} <span style={{color: '#888'}}>({p.score} pts)</span></li>))}</ul>{players.length > 0 && players[0].id === socket.id && (<button onClick={handleStartGame} disabled={players.length < 2} style={styles.button}>Start Game</button>)}{players.length < 2 && players.length > 0 && players[0].id === socket.id && <p>Need at least 2 players to start.</p>}</div>;
            }
          })()}
        </div>
      </div>
